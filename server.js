...
mongoose.connect(
  "mongodb+srv://abubakker13:DiscussionForum1@discussionforumcluster.onqjk.mongodb.net/myFirstDatabase?retryWrites=true&w=majority",
  {
...
var allowedOrigins = [
  "http://localhost:3000",
  "https://discussion-forum-frontend.herokuapp.com",
];
app.use(
  cors({
...
app.post("/login", (req, res, next) => {
  passport.authenticate("local", (err, user, info) => {
    if (err) throw err;
    if (!user) res.send({ message: "User doesnt exists", username: "" });
    else {
      req.logIn(user, (err) => {
        if (err) throw err;
        const result = {
